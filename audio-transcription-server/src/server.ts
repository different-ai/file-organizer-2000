...
app.post('/transcribe', upload.single('audio'), async (req: Request, res: Response) => {
  ...
  try {
    ...
    fs.unlinkSync(req.file.path);
    res.status(500).json({ error: 'Error transcribing audio', details: (error as Error).message });
  }
});
...
async function splitAudio(inputPath: string, outputPath: string, start: number, end: number): Promise<void> {
